...
async function test() {
  ...
  const [simpIndices] = MeshoptSimplifier.simplify(indices, pos, 3, 60000 * 3, 0.02);
  console.timeEnd('Simplify');

  console.time('Face normals');
  ...
  console.timeEnd('Face normals');

  console.time('Build edge graph');
  ...
  for (const edge of edgeMap.values()) {
    if (edge.tris.length === 1) {
      ...
      if (dot < 0.88) {
        creaseEdges.push(edge);
      }
    }
  }
  console.timeEnd('Build edge graph');

  console.log('Total unique edges:', edgeMap.size);
  console.log('Crease / Feature edges found:', creaseEdges.length);
  console.log('Boundary edges found:', boundaryEdges.length);
}
